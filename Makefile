...
COMMON_CONSTRAINTS_TXT=requirements/constraints.txt
...
upgrade: piptools $(COMMON_CONSTRAINTS_TXT) ## update the requirements/*.txt files with the latest packages satisfying requirements/*.in
	sed 's/Django<4.0//g' requirements/constraints.txt > requirements/constraints.tmp
	mv requirements/constraints.tmp requirements/constraints.txt
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it edx_exams.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it edx_exams.db mysql -u root edx_exams
